...
import edu.ouhk.comps380f.dao.LabRepository;
...
public class LabController {
    ...
    private Map<Long, Lab> ticketDatabase = new LinkedHashMap<>();

    @RequestMapping(value = {"", "lab"}, method = RequestMethod.GET)
    public ModelAndView list(Principal principal) {
        ModelAndView mav = new ModelAndView("lab");
        mav.addObject("lablist", labRepo.findAll());
    ...
    public ModelAndView view2(@PathVariable("ticketId") int ticketId) {

        ModelAndView modelAndView = new ModelAndView("view2");
        modelAndView.addObject("labInfo", labRepo.findByLabId(ticketId));
    ...
    public View deleteTicket(@PathVariable("ticketId") int ticketId) {
        labRepo.deleteByLabId(ticketId);

        return new RedirectView("/lab", true);
    }

    @RequestMapping(value = "view2/{labId}/deleteReply/{replyId}", method = RequestMethod.GET)
    public View deleteReply(@PathVariable("replyId") int replyId, @PathVariable("labId") int labId) {
        replyRepo.deleteByReplyId(replyId);
        return new RedirectView("/lab/view2/{labId}", true);
    }
    ...
    public View reply(@PathVariable("ticketId") int ticketId, replyForm form, Principal principal) throws IOException {
        ...
        return new RedirectView("/lab/view2/" + ticketId, true);
    }
    ...
    public View create(Form form, Principal principal) throws IOException {
        ...
        for (MultipartFile filePart : form.getAttachments()) {
            ...
            int topicId = labRepo.createLab(ticket);
            labRepo.createAttachment(ticket, topicId);
 ...
 /*this.ticketDatabase.put(ticket.getId(), ticket);*/
    ...
    public View download(@PathVariable("ticketId") int ticketId, @PathVariable("attachment") String name) {
        Lab lab = labRepo.findByLabId(ticketId);
        if (lab != null) {
            Attachment attachment = lab.getAttachment(name);
            if (attachment != null) {
